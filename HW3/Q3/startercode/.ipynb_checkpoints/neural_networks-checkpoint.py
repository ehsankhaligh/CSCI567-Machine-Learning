...
class relu:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        # Apply the gradient only where the corresponding value of X was greater than 0
        backward_output = grad * self.mask
        
        return backward_output

"""
Stochastic Gradient Descent (SGD): In traditional stochastic gradient descent, instead of computing the gradient of the loss function with respect to the parameters using the entire dataset, you compute it using only one randomly chosen example from the dataset. This makes the gradient noisy but allows for faster updates.

Mini-Batch Processing: In mini-batch processing, instead of processing the entire dataset at once, you divide it into smaller subsets called mini-batches. Each mini-batch contains a fixed number of examples. This allows for more efficient computation, as it takes advantage of parallelism and reduces memory requirements.

Combining SGD with Mini-Batches: In mini-batch SGD, you compute the gradient of the loss function with respect to the parameters using a mini-batch of examples instead of just one example. This reduces the noise in the gradient estimate compared to using just one example (as in traditional SGD), leading to more stable updates. Additionally, it takes advantage of the computational benefits of mini-batch processing.
"""
...
def backward_pass(model, x, a1, h1, a2, y):
    ...
    ######################################################################################
    # Call the backward methods of every layer in the model in reverse order.
    # We have given the first and last backward calls (above and below this TODO block).
    ######################################################################################

    #we call the backward methods of each layer in the model in reverse order compared to the forward pass. We start by computing the gradient of the loss with respect to the output of the last layer (a2). Then, we propagate this gradient backwards through the network, calling the backward methods of each layer with the appropriate inputs and gradients. Finally, we obtain the gradient of the loss with respect to the input x by backpropagating through the first layer (L1).
